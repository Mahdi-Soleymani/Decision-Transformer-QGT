...
def generate_random_entropy_sample(k, max_len, pad_scalar_val, pad_vec_val, x, x_half):
    x = x.reshape(-1, 1)
    x_half = x_half.reshape(-1, 1)

    model = Model("RandEntropy_ILP")
    ...
    q, r, rwrd, entropy_list = [], [], [-1], [ np.log2(model.SolCount)]
    ...
    while not is_solved:
        ...
        selected_variables = []
        # entropy = np.log2(model.SolCount) if model.status == GRB.OPTIMAL and model.SolCount > 0 else 0.0
        ...
        model.addConstr(sum(selected_variables) == new_result, name=f"c{num_of_constraints}")
        ...
        if model.status == GRB.OPTIMAL:
            if model.SolCount <= 1:
                ...
                rwrd.append(-1)
            entropy_list.append(np.log2(model.SolCount))
        else:
            rwrd.append(0)
            is_solved = True
            entropy_list.append(np.log2(model.SolCount))
    ...
    with h5py.File(file_name, 'w') as f:
        ...
        while True:
            ...
            for _ in range(num_samples_per_worker):
                q, r, rtg, mask_length, d_bound, entropy = generate_random_entropy_sample(k, max_len, pad_scalar_val, pad_vec_val, x, x_half)
